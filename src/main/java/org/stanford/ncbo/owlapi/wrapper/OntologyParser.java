...
public class OntologyParser {
    ...
    private OWLOntology findMasterFile() {
        ...
        if (parserInvocation.getInputRepositoryFolder() == null) {
            try {
                ...
                ontology = addImportsAxiomsToOriginalOntology( sourceOwlManager, ontology);
                return ontology;
            } catch (OWLOntologyCreationException e) {
                log.error(e.getMessage());
    ...
    private OWLOntology addImportsAxiomsToOriginalOntology( OWLOntologyManager sourceOWLManager, OWLOntology sourceOntology) {

        if (sourceOntology.getImportsDeclarations().isEmpty()) return sourceOntology;
        ...
        for (OWLImportsDeclaration importDeclaration : sourceOntology.getImportsDeclarations()) {
            ...
            sourceOWLManager.addAxioms(sourceOntology, importedOntology.getAxioms());

            manager.removeOntology(importedOntology);
        }

        sourceOWLManager.addAxioms(sourceOntology, sourceOntology.getAxioms());
